...
- Logic tests confirm fixture validation, large CSV review filters, preset trust metadata and fingerprints, local OCR engine planning, provider payload fixture coverage, scheduler recipes, PDF fallback diagnostics, and self-hosted send guard behavior. `npm run notify:smoke` confirms loopback ntfy/Gotify send behavior.
