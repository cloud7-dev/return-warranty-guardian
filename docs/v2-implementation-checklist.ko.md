...
- ntfy/Gotify/Apprise용 self-hosted 알림 payload 초안 JSON 내보내기
...
4. **서버 없는 알림 경험 고도화**
   ...
   - 현재 구현: ntfy/Gotify/Apprise에 사용자가 직접 적용할 수 있는 self-hosted 알림 payload/curl 초안 내보내기
   - 남은 구현: 더 세밀한 스누즈 시간 선택, 실제 전송이 필요한 경우의 opt-in self-hosted 설정 저장, 플랫폼별 알림 가이드 강화
...
V2의 미해결 불편사항은 제품/문서/데이터 방향에 반영되었고, 1번 실제 첨부 파일 저장, 2번 CSV import preset 확대와 fixture corpus/익명화 도구의 1차 범위, 3번 로컬 OCR/정책 추출과 fixture corpus의 1차 범위, 4번 서버 없는 알림 경험/캘린더 가이드/스누즈/self-hosted payload의 1차 범위, 5번 클레임 패킷 HTML/PDF는 사용 가능한 수준으로 보강되었습니다. 남은 큰 묶음은 익명화된 실제 샘플 기반 import/OCR fixture 확대, 압축/스캔 PDF OCR, 더 세밀한 스누즈 시간 선택, 실제 opt-in self-hosted 전송 설정, 암호화 백업, polished PWA입니다.
