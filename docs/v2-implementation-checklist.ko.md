...
- CSV import review checklist, import report JSON, CSV preset bundle JSON 내보내기/가져오기
...
- ntfy/Gotify/Apprise용 self-hosted 알림 설정 로컬 저장과 payload 초안 JSON 내보내기
...
2. **CSV import**
   ...
   - 현재 구현: import review checklist로 필수 매핑/중복/오류/증빙 누락을 확인하고, CSV preset bundle JSON을 내보내기/가져오기
   - 남은 구현: 익명화된 실제 사용자 샘플 기반의 카드사/판매처별 fixture 확대, preset bundle 서명/버전 호환성 검증, 더 세밀한 수동 리뷰 워크플로
...
3. **로컬 OCR과 영수증/정책 추출**
   ...
   - 현재 구현: 사용자가 확인해서 적용하는 정책 템플릿으로 반품/환불/보증 기본값, 증빙 요구사항, 국가/관할권 면책 메모를 채움
   - 현재 구현: HTML 이메일 영수증, PDF text operator, 압축/스캔 PDF fallback, 정책 템플릿 synthetic fixture corpus로 회귀 테스트
   - 남은 구현: 모든 브라우저에서 동작하는 번들형 이미지 OCR 엔진, 실제 스캔 PDF OCR, 실제 판매처 정책 fixture 확대, 국가별 정책 출처/버전 관리
...
4. **서버 없는 알림 경험 고도화**
   ...
   - 남은 구현: 실제 백그라운드 전송을 위한 별도 opt-in runner, 플랫폼별 알림 가이드 강화, self-hosted provider별 dry-run 검증
...
V2의 미해결 불편사항은 제품/문서/데이터 방향에 반영되었고, 1번 실제 첨부 파일 저장, 2번 CSV import preset 확대/리뷰 체크리스트/preset bundle 공유의 1차 범위, 3번 로컬 OCR/정책 추출/fallback/정책 면책 노트의 1차 범위, 4번 서버 없는 알림 경험/캘린더 가이드/세분화 스누즈/self-hosted 설정 저장의 1차 범위, 5번 클레임 패킷 HTML/PDF는 사용 가능한 수준으로 보강되었습니다. 남은 큰 묶음은 익명화된 실제 샘플 기반 import/OCR fixture 확대, 번들형 크로스브라우저 OCR과 실제 스캔 PDF OCR, self-hosted background runner, 암호화 백업, polished PWA입니다.
