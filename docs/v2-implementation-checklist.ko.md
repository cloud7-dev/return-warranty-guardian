...
- 클레임 번들 JSON: 구매 레코드, 마감 계산, Markdown 증빙팩, claim HTML, 로컬 첨부 data URL 포함
...
2. **CSV import**
   ...
   - 남은 구현: 실제 판매처/카드사별 preset 확대, 대량 import 리포트 내보내기, 사용자 preset 저장
...
5. **클레임 패킷 HTML/PDF**
   - 현재 구현: 출력용 HTML 클레임 패킷 생성, 브라우저 인쇄 대화상자를 통한 PDF 저장, 문서명/첨부 링크/이미지 미리보기/시리얼/수리 이력/제출 전 확인 메모 포함, claim bundle JSON으로 증빙 데이터 묶음 내보내기
   - 남은 구현: ZIP export bundle, 판매처 제출용 템플릿, chargeback/수리접수 템플릿, 첨부 파일 용량 최적화
...
V2의 미해결 불편사항은 제품/문서/데이터 방향에 반영되었고, CSV import preview/수동 매핑/중복 감지, 로컬 HTML 영수증 및 지원 브라우저 이미지 OCR, 클레임 HTML/PDF 첨부 미리보기, claim bundle JSON까지 구현되었습니다. 다만 브라우저 독립형 OCR 엔진, 실제 판매처/카드사별 preset 확대, ZIP export bundle, 암호화 백업, polished PWA는 아직 남아 있으므로 V2 완료가 아니라 V2 핵심 흐름을 계속 닫아가는 단계입니다.
