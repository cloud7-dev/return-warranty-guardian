...
Local OCR/text extraction is handled in the browser. Text, CSV, HTML/email, and simple PDF text-operator paths are file reads; image OCR uses browser-local `TextDetector` only when the current browser supports it. `pdfExtractionDiagnostics` classifies PDFs as text-operator, scanned/compressed, or plain fallback and records image/compression/encryption signals; compressed or scanned PDFs that do not expose text operators produce a no-upload fallback notice instead of calling cloud OCR.
