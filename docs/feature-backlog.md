...
7. **Local notification upgrade**
   ...
   - Provider-specific endpoint fixtures, scheduler recipes, platform fallback guidance, an operating guide, local loopback smoke tests, opt-in public endpoint smoke mode, and manual GitHub Actions smoke workflow now cover ntfy and Gotify send paths plus Apprise dry-run plans.
