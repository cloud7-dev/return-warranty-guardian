...
The canonical repository also has public ntfy smoke repository variables configured for the scheduled workflow. The committed fixture record stores only the generated timestamp, loopback statuses, public status, provider name, and endpoint host hash.
