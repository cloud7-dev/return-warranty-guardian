...
- Extracts local text, CSV, HTML/email receipts, simple PDF text operators, scanned/compressed PDF fallback diagnostics, matching `.ocr.txt` sidecars for scanned PDFs, and supported browser-local image OCR into the receipt parser without upload.
...
Local OCR/text extraction is intentionally no-upload. The current implementation handles text, CSV, HTML/email receipts, simple PDF text operators, scanned/compressed PDF fallback diagnostics, matching `.ocr.txt` sidecars selected together with scanned PDFs, and image OCR through a local OCR adapter when the browser exposes `TextDetector` or a future bundled worker. If image OCR is not available in the current browser, the app keeps the flow local and asks the user to paste receipt text instead of calling a cloud OCR service.
